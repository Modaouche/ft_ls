...
/*   Updated: 2018/10/06 17:43:21 by modaouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_next_line(const int fd, char **line)
{
	...
	if (!s_line[fd])
	{
		if (!(s_line[fd] = ft_strnew(BUFF_SIZE + 1)))
			return (-1);
		if ((ret = read(fd, s_line[fd], BUFF_SIZE)))
			s_line[fd][ret] = '\0';
	}
	if (!(buff = ft_strnew(BUFF_SIZE + 1)))
		return (-1);
	while ((ret = read(fd, buff, BUFF_SIZE)) && ft_util(fd, s_line, ret, buff))
